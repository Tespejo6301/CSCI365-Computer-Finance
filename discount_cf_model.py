    ...
    def calc_fair_value(self):
       ...
       - 2. Get the Free Cash flow
        ...
        try:
            ...
            for i in range(1, 6):
                dcf += free_cash_flow * (1+ self.short_term_growth_rate)**i * (DF) ** i
                cf5 = free_cash_flow * (1+ self.short_term_growth_rate)** 5 
            for i in range(1, 6):
                dcf += (cf5 * (1+ self.medium_term_growth_rate) ** i) * (DF) ** i + 5 
                cf6_10 = cf5 * (1+ self.short_term_growth_rate) ** 5 
            for i in range(1, 11):
                dcf += (cf6_10 * (1+ self.long_term_growth_rate )**i) * (DF) ** i + 10          
        
            pv = self.stock.get_cash_and_cash_equivalent() - self.stock.get_total_debt() + dcf
            result = pv / num_shares_outstanding
